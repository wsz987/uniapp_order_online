<template>
	<view class="order-card">
		...
		<view class="content">
		    <u--image :src="item" :lazy-load="true" radius='5'
		    	width='90' mode="widthFix" v-for="item in getOrderImg(info.order_data)">
		    </u--image>
		</view>
...
<script>
	export default {
		...
		computed:{
			totalCount(){
				return this.info.order_data.reduce((acc,{count})=>acc+count,0)
			},
			getOrderImg(){
				return data => {
					let showNumber = 3
